...
func TestITCreateExpense(t *testing.T) {
	...
	go func(e *echo.Echo) {
		...
		e.GET("/expenses", h.CreateExpenseHandler)
	...
	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("http://localhost:%v/expenses", serverPort), strings.NewReader(reqBody))
